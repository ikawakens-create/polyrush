...
void main() {
  ...
  group('A. translateCells', () {
    ...
    test('正の移動', () {
      expect(translateCells(cells, 3, 4), [
        (4, 6),
        (4, 7),
        (5, 6),
    ...
    test('負の移動', () {
      expect(translateCells(cells, -1, -2), [
        (0, 0),
        (0, 1),
        (1, 0),
  ...
  group('B. placedCellsAt', () {
    test('L 字ピースを (2,3) に配置', () {
      ...
      expect(placedCellsAt(normalized, (2, 3)), [
        (2, 3),
        (3, 3),
        (3, 4),
    ...
    test('単一セルを (0,0) に配置', () {
      expect(placedCellsAt(const [(0, 0)], (0, 0)), [
        (0, 0),
  ...
  group('D. isComplete', () {
    ...
    test('配置なし（placed なし）→ false', () {
      expect(isComplete([], frame), isFalse, reason: 'ピースが1つも置かれていないので false');
    });
