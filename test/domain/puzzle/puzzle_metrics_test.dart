...
void main() {
  ...
  group('PuzzleMetrics unit tests', () {
    ...
    test('stripes: rotationalSymmetry90 == 0.0', () {
      final m = computePuzzleMetrics(stripes);
      expect(m.rotationalSymmetry90, isNotNull, reason: '4x4 は正方形');
      expect(
        m.rotationalSymmetry90!,
        closeTo(0.0, 0.001),
    ...
    test('stripes: straightCutSeparable == true', () {
      final m = computePuzzleMetrics(stripes);
      expect(m.straightCutSeparable, isTrue, reason: '行境界がすべてカット可能');
    });
    ...
    test(
      'orientationUsageRatio: 0.5 when half blocks use non-source orientation',
      () {
        final srcA = _poly('A4', [(0, 0), (0, 1), (1, 0), (1, 1)]);
        // orientation に同じインスタンスを使う → source == orientation
        final blockSame = PlacedBlock(
          source: srcA,
          orientation: srcA,
  ...
  group('PuzzleMetrics report', () {
    test(
      'metrics report for easy/normal/hard seeds 1..100',
      () {
        final difficulties = [
          Difficulty.easy,
          Difficulty.normal,
        ...
        for (final diff in difficulties) {
          ...
          for (int seed = 1; seed <= 100; seed++) {
            final result = CompactPuzzleGeneratorV3.generate(
              difficulty: diff,
              seed: seed,
            );
            switch (result) {
          ...
          final sym180avg =
              metrics
                  .map((m) => m.rotationalSymmetry180)
                  .reduce((a, b) => a + b) /
              n;
          final sym90vals = metrics
              .map((m) => m.rotationalSymmetry90)
              .whereType<double>()
              .toList();
          final sym90avg = sym90vals.isNotEmpty
              ? sym90vals.reduce((a, b) => a + b) / sym90vals.length
              : double.nan;
          final sym90nullRate = (n - sym90vals.length) / n;
          final sepRate =
              metrics.where((m) => m.straightCutSeparable).length / n;
          final protAvg =
              metrics.map((m) => m.protrusionRatio).reduce((a, b) => a + b) / n;
          final fillAvg =
              metrics.map((m) => m.fillRatio).reduce((a, b) => a + b) / n;
          final oriAvg =
              metrics
                  .map((m) => m.orientationUsageRatio)
                  .reduce((a, b) => a + b) /
              n;
          final pcAvg =
              metrics.map((m) => m.pieceCount).reduce((a, b) => a + b) / n;

          print('');
          print('=== ${diff.name.toUpperCase()} (n=$n) ===');
          print('sym180_avg        : ${sym180avg.toStringAsFixed(3)}');
          print(
            'sym90_avg(non-null): ${sym90avg.isNaN ? "N/A" : sym90avg.toStringAsFixed(3)}',
          );
          ...
          for (final entry in withSym90.take(3)) {
            print(
              '  seed=${entry.$2} sym90=${entry.$1.toStringAsFixed(3)} sym180=${entry.$3.rotationalSymmetry180.toStringAsFixed(3)} sep=${entry.$3.straightCutSeparable} prot=${entry.$3.protrusionRatio.toStringAsFixed(3)}',
            );
          }
          ...
          for (final entry in nonSep.take(3)) {
            print(
              '  seed=${entry.$2} prot=${entry.$1.toStringAsFixed(3)} sym180=${entry.$3.rotationalSymmetry180.toStringAsFixed(3)} fill=${entry.$3.fillRatio.toStringAsFixed(3)}',
            );
          }
        ...
        for (final seed in [1, 2]) {
          ...
          if (result case Ok(:final value)) {
            ...
            for (int i = 0; i < 2 && i < blocks.length; i++) {
              final b = blocks[i];
              print(
                '  seed=$seed block[$i] source.cells=${b.source.cells} ori.cells=${b.orientation.cells} same=${b.source == b.orientation}',
              );
            }
          }
        }
